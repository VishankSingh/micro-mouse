...
void drive(int c) // 1 2 3 4 ; drive forward, left, right, stop;
{
  ...
  if (n == 1) { //drive forward
    Serial.println("forward");
    isDriving = true;
  } else if (n == 2) { //turn left
    Serial.println("left");
    targetAngle += 90;
    if (targetAngle > 180){
      targetAngle -= 360;
    ...
    isDriving = false;
  } else if (n == 4) { //stop or brake
    Serial.println("stop");
    isDriving = false;
  } else if (n == 5) { //print information. When car is stationary, GyroX should approx. = 0. 
    Serial.print("angle: ");
      ...
      if (isDriving) {
        if (abs(targetAngle - angle) < 3){
          if (countStraight < 20){
            ...
            countStraight = 0;
            equilibriumSpeed = rightSpeedVal; //to find equilibrium speed, 20 consecutive readings need to indicate car is going straight
...
void controlSpeed (){//this function is called by driving ()
  ...
  if (round(targetGyroX - GyroX) == 0){
    ...
    rightSpeedVal = changeSpeed(rightSpeedVal, +1); //would increase GyroX
  } else {
...
void rotate (){//called by void loop(), which isDriving = false
  ...
  if (abs(deltaAngle) <= 1){
    ...
    if (round(targetGyroX - abs(GyroX)) == 0){
      ...
      rightSpeedVal = changeSpeed(rightSpeedVal, -1); //would increase abs(GyroX)
    } else {
      rightSpeedVal = changeSpeed(rightSpeedVal, +1);
    }
    leftSpeedVal = rightSpeedVal;
    analogWrite(rightSpeed, rightSpeedVal);
    analogWrite(leftSpeed, leftSpeedVal);
  }
}   
...
void forward_s(){ //drives the car forward, assuming leftSpeedVal and rightSpeedVal are set high enough
...
void left_s(){ //rotates the car left, assuming speed leftSpeedVal and rightSpeedVal are set high enough
  digitalWrite(right1, LOW);
